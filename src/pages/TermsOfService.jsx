...
export default function TermsOfService() {
    ...
    useEffect(() => {
        const loadContent = async () => {
            try {
                const contentBlock = await ContentBlock.filter({ page_key: 'terms_of_service_content' });
                if (contentBlock.length > 0) {
                    setContent(contentBlock[0].content);
                } else {
                    setContent('<h1>Terms of Service</h1><p>Content not found. Please contact support.</p>');
                }
            } catch (err) {
